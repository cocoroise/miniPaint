...
class Selection_class extends Base_tools_class {
  ...
  mousemove(e) {
    ...
    if (
      (e.type == "mousedown" && (mouse.valid == false || mouse.click_valid == false)) ||
    ...
    if (this.type == "create") {
      ...
      this.tmpCanvasCtx.drawImage(
        layer.link,
        Math.round(from_x - layer.x),
        Math.round(from_y - layer.y),
        width,
        height,
        Math.round(selection.x - layer.x),
        Math.round(selection.y - layer.y),
        width,
        height
      )
