...
class Erase_class extends Base_tools_class {
  ...
  dragMove(event) {
    ...
    if (params.circle == true) _this.show_mouse_cursor(mouse.x, mouse.y, params.size, "circle")
    else _this.show_mouse_cursor(mouse.x, mouse.y, params.size, "rect")
  }
  ...
  mousedown(e) {
    ...
    // 应该把除了背景的东西都画在tmpcanvas上面 遍历调用render_object方法
  ...
  erase_general(ctx, type, mouse, size, strict, is_circle) {
    ...
    if (type == "move" && is_circle == true && mouse_last_x != false && mouse_last_y != false) {
      if (strict == false && is_circle == true) {
        ...
        if (alpha < 255) radgrad.addColorStop(0, "rgba(255, 255, 255, " + alpha / 255 / 10 + ")")
        else radgrad.addColorStop(0, "rgba(255, 255, 255, 1)")
        radgrad.addColorStop(1, "rgba(255, 255, 255, 0)")
