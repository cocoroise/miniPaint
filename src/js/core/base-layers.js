...
class Base_layers_class {
  ...
  render_object(ctx, object) {
    ...
    if (filter_code != "") ctx.filter = filter_code
  ...
  async insert(settings, can_automate = true) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      if (layer.id >= _this.auto_increment) _this.auto_increment = layer.id + 1
  ...
  autoresize(width, height, layer_id, can_automate = true) {
    ...
    if (layer_id == undefined) layer_id = config.layer
    ...
    if (width > config.WIDTH || height > config.HEIGHT) {
      ...
      if (width > config.WIDTH) config.WIDTH = parseInt(width)
      if (height > config.HEIGHT) config.HEIGHT = parseInt(height)
    }
  ...
  delete(id, force) {
    ...
    if (config.layer.id == id) {
      ...
      if (config.layer == null) config.layer = this.find_previous(id)
    }
  ...
  toggle_visibility(id) {
    ...
    if (link.visible == false) link.visible = true
