  ...
  render_general_select_param(key, events) {
    ...
    if (layer != undefined) {
      ...
      if (layer.params[key] == null) {
        ...
        if (typeof layer.params[key] == "object") target.value = layer.params[key].value
        //legacy
  ...
  render_reset(events) {
    ...
    if (events) {
      ...
      document.getElementById("reset_x").addEventListener("click", function(e) {
        if (config.layer.x != null) config.layer.x = 0
        config.need_render = true
      })
      document.getElementById("reset_y").addEventListener("click", function(e) {
        if (config.layer.x != null) config.layer.y = 0
        config.need_render = true
      })
          ...
          params: [
            {
              name: "text",
              title: "Text:",
              value: config.layer.params.text || "",
              type: "textarea"
            }
          ],
