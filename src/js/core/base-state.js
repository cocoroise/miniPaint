...
class Base_state_class {
  ...
  optimize() {
    ...
    if (total_megapixels > 100) {
      ...
      if (this.enabled == true) alertify.warning("Undo disabled.")
      this.enabled = false
      this.layers_archive = []
    } else {
      //enabled
      if (this.enabled == false) alertify.success("Undo enabled.")
      this.enabled = true

      if (total_megapixels > 50) {
        //1 undo level
        if (this.levels > 1) alertify.warning("Undo levels changed to 1.")
        this.levels = 1
        this.layers_archive = [this.layers_archive[0]]
      } else {
        //OK
        if (this.levels == 1) alertify.success("Undo levels restored to " + this.levels)
