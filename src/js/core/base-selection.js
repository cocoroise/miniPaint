...
class Base_selection_class {
  ...
  draw_selection() {
    ...
    function corner(x, y, dx, dy, name) {
      ...
      if (config.ZOOM < 1) _this.ctx.lineWidth = 2
      _this.ctx.beginPath()
      _this.ctx.arc(
  ...
  selected_object_actions(e) {
    ...
    if (
      (e.type == "mousedown" && config.mouse.valid == false) ||
    ...
    if (e.type == "mousemove" && this.mouse_lock == "selected_object_actions") {
      ...
      if (e.buttons == 1) {
        ...
        if (type.indexOf("top") >= 0) var height = this.click_details.height - dy
        if (type.indexOf("left") >= 0) var width = this.click_details.width - dx

        if (
          (type.indexOf("_") >= 0 && (settings.keep_ratio == true && is_ctrl == false)) ||
        ...
        if (type.indexOf("left") >= 0 || type.indexOf("right") >= 0) settings.data.width = width
        if (type.indexOf("top") >= 0 || type.indexOf("bottom") >= 0) settings.data.height = height
    ...
    for (var i in this.selected_obj_positions) {
      ...
      if (
        mouse.x >= positions.x &&
        mouse.x <= positions.x + positions.size &&
        mouse.y >= positions.y &&
        mouse.y <= positions.y + positions.size
      ) {
        //match
        if (e.type == "mousedown") {
          if (e.buttons == 1) {
